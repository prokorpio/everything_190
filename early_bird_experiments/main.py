...
if args.dataset == 'cifar10':
    ...
    train_loader = torch.utils.data.DataLoader(
        ...
        num_workers=0, pin_memory=True)# jeff: changed num_work from 16 to 0
...
def updateBN(model=model): # jeff: default model is the global model
...
class EarlyBird():
    def __init__(self, percent, time_per_layer, epoch_keep=5):
        ...
        if args.timerank:
            ...
            self.time_per_layer = [0]*len(time_per_layer) #So that an == 1
    ...
    def pruning(self, model, percent):
        ...
        an = torch.zeros(total) #alpha scalar for time
        index = 0
        layer = 0
        for m in model.modules():
            if isinstance(m, nn.BatchNorm2d):
                ...
                an[index:(index+size)] = torch.ones((size))*(1 - float(self.time_per_layer[layer]))
                index += size
                layer += 1
        ...
        thre = y[thre_index] #dependent on an
        ...
        for k, m in enumerate(model.modules()):
            if isinstance(m, nn.BatchNorm2d):
                ...
                weight_copy = weight_copy*(1-float(self.time_per_layer[layer])) #incorporate dependency on an
...
#Count number of convs
num_of_convs = 0
for child in model.feature.children():
    if isinstance(child, nn.Conv2d):
        num_of_convs += 1

#Get time per layer
import parse_trace_file as ptf
time_per_layer = ptf.get_time('tracing.json', num_of_convs=num_of_convs)
print('layer_times: ',time_per_layer)
...
flag_50 = True
flag_70 = True
early_bird_30 = EarlyBird(0.3, time_per_layer = time_per_layer)
early_bird_50 = EarlyBird(0.5, time_per_layer = time_per_layer)
early_bird_70 = EarlyBird(0.7, time_per_layer = time_per_layer)

#To do: Save epoch found in a text files
EB_epoch_record = []
#To do: Save EB location in a way that you can refer to it in the next .sh
# handled in
import sys
