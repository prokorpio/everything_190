...
if args.dataset == "imagenet":
    ...
    model = BasicCNN(dataset=args.dataset, depth=args.depth)
...
if args.dataset == "imagenet":
    ...
    newmodel = BasicCNN(dataset=args.dataset, cfg=cfg)
...
for [m0, m1] in zip(model.modules(), newmodel.modules()):
    if isinstance(m0, nn.BatchNorm2d):
        ...
        idx0 = np.squeeze(np.argwhere(np.asarray(start_mask.cpu().numpy())))
        print(idx0, "IDX0 linear")
        if idx0.size == 1:
            idx0 = np.resize(idx0, (1,))
        m1.weight.data = m0.weight.data[:, idx0].clone()
        m1.bias.data = m0.bias.data.clone()
        print(m0.weight.data.shape, "Shape of weight originally")
        print(m1.weight.data.shape, "Shape of weight")
