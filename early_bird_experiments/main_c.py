...
#To do: Add code for measuring time_per_epoch.
import time
time_per_epoch = []
...
np.savetxt(os.path.join(args.save, 'record.txt'), history_score, fmt = '%10.5f', delimiter=',')
ave_time = sum(time_per_epoch)/len(time_per_epoch)
#print(time_per_epoch)
time_per_epoch.append(ave_time) # last time is the average 
np.savetxt(os.path.join(args.save, 'time.txt'), time_per_epoch)
