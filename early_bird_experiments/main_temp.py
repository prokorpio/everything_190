...
class EarlyBird():
    def __init__(self, percent, epoch_keep=5, path_to_energy_file = 'energy.csv'):
        self.percent = percent
        self.epoch_keep = epoch_keep
        self.masks = []
        self.dists = [1 for i in range(1, self.epoch_keep)]
        self.energy_layer = er.energyreader(path_to_energy_file) #added
        self.energy_layer = er.minmaxer(self.energy_layer) #added
        self.energy_layer = self.energy_layer + 1
        print("This is my energy_per_layer", self.energy_layer) #added
    ...
    def pruning(self, model, percent):
        ...
        for m in model.modules():
            if isinstance(m, nn.BatchNorm2d):
                ...
                an[index:(index+size)] = torch.ones((size))*float(self.energy_layer[layer]) #added
                index += size
                layer += 1 #added
        ...
        for k, m in enumerate(model.modules()):
            if isinstance(m, nn.BatchNorm2d):
                ...
                #weight_copy = weight_copy * float(self.energy_layer[layer])
...
#time_per_layer = ptf.get_time(num_of_convs=16) #16 depends on which model
