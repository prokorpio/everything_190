...
def step_from(mask, num_to_flip=2):
    """Transitions from one state (mask) to another.

    Flips equal amounts of zeros and ones to maintain sparsity.

    Args:
        mask (1d tensor): binary mask to derive a new mask from
        num_to_flip (int): half the Hamming distance bet prev and new mask

    Returns:
        new_mask (1d tensor): mask w/ same sparsity but different permuation
    """
...
def is_in_memory(new_mask, mask_memory):
    """Checks if new_mask is inside memory"""
    is_in_mem_flag = 0
...
def extract_feature_map_sizes(model, input_data_shape, device=None):
    ...
    Args:
        ...
        input_data_shape ([C, H, W]): dataset dimensions

    Returns:
        fmap_sizes_dict ({layer_name : [feat_size]}) : feature map sizes
    ...
    def _register_hook(submodule):
        ...
        def _hook(submodule, input, output):
            ...
            if type_str in ["Conv2d", "Linear"]:  # Linear is just conv w/
                ...
                fmap_sizes_dict[module_id] = {
                    "in_fmap_size": in_fmap_size,
                    "out_fmap_size": out_fmap_size,
                }

        if (
            not isinstance(submodule, torch.nn.Sequential)
...
def get_network_def_from_model(model, input_data_shape):
    """Gets layer properties of a model.
...
def compute_weights_and_flops(network_def):
    ...
    for layer_name in network_def.keys():
        ...
        layer_num_weights = (
            (
                network_def[layer_name]["num_out_channels"]
